...
void SSLSocket::InitContext( bool serverContext ) {
   ...
   if ( serverContext ) {
      method = TLS_server_method();  // Create the instance for server.
   } else {
      method = TLS_client_method();  // Create the instance for client.
   }
   ...
   context = SSL_CTX_new( method ); // Create new context instance.
   if ( nullptr == context ) {
      throw std::runtime_error( "SSLSocket::InitContext( bool )" );
   }
...
int SSLSocket::MakeConnection( const char * hostName, int port ) {
   ...
   int st = this->VSocket::EstablishConnection( hostName, port );
   if (st < 0) {
...
int SSLSocket::MakeConnection( const char * host, const char * service ) {
   int st = this->VSocket::EstablishConnection(host, service);
   if (st <0) {
